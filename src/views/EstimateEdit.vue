...
import { SelectOption } from '@/domain/models/selectOption';
...
export default class Estimate extends Vue {
    ...
    private productOptions: SelectOption[] = [];
    ...
    initProductOptions() {
        if (this.IsMaster) {
            this.productOptions = [];
            let emptyOption = { value: '', text: 'Projekt auswählen' };
            this.productOptions.push(emptyOption);
            for (let p of this.Master.products) {
                let option = {
                    value: p.id.toString(),
                    text: p.name
                }
            ...
            this.selectedProduct = emptyOption.value;
        }
    }
